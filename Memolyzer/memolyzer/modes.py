...
def print_message(message_type: str, text: str, limit_lines: bool = False):
    ...
    if not limit_lines:
        ...
        if len(text.split("\n")) > 20:
            truncated_text = "\n".join(text.split("\n")[:20])
            print(
                f"{color_map[message_type.lower()]}{truncated_text}\n...{ColorStruct.RESET}"
            )
        else:
            print(f"{color_map[message_type.lower()]}{text}{ColorStruct.RESET}")
...
class Modes:
    ...
    def run_modes(self):
        ...
        mode_functions = {
            "symbol_info": self.get_symbol_info,  # tamamlandı
    ...
    def get_layers_mem_usage(self):
        ...
        merged_df["Type"] = merged_df.apply(
            lambda x: (
                arch_files[arch_files["File"] == x["From archive"]]["Type"].values[0]
        ...
        for file_name in merged_df["File"]:
            ...
            for chip in grouped_file_info_df.groups.keys():
                size = (
                    grouped_file_info_df.get_group(chip)["Size (MAU)"]
                ...
                layers_df = pd.concat(
                    [
                        ...
                        pd.DataFrame(
                            [
                                [
                                    merged_df[merged_df["File"] == file_name][
                                        "Type"
                                    ].values[0],
                                    chip,
                                    size,
    ...
    #This function gets one of the file_info_table and
    #basically add it to one extra column to specify symbol in the section variable or function.
    def classify_sections_as_variable_or_function(self, file_info_table):
        file_info_table["SymbolType"] = file_info_table["[in] Section"].apply(
            lambda x: (
                "Function"
                if x.lstrip().startswith(".text.")
                else "Variable" 
            )
        )        
    ...
    def memory_validation_check_expected_and_finding(self, expected_list, finding_list):
        ...
        for i in finding_list:
            if(i in expected_list):
                #info_msg += "{0} is in expected list".format(i) + "\n"
                continue
            info_msg += "{0} is not in expected list".format(i) + "\n"
            is_valid = False

        if(is_valid):
            for j in expected_list:
                if(j in finding_list):
                    #info_msg += "{0} is in finding list".format(j) + "\n"
                    continue
                info_msg += "{0} is not in founded memory areas in map file".format(j) + "\n"
    ...
    def get_mem_section_check(self):
        excel_df = pd.read_excel(
            "mode5_config.xlsx"
        )  # has Type	File Name	Expected Memory (Chip) columns expected memory can be multiple like mpe:dsram1, mpe:dsram2
        # read file names from excel and use get_file_info for each file
        for index, file_name in excel_df["File Name"].items():
            
            file_info_df = self.get_file_info(file_name)
            #file_info_df = file_info_df.dropna()
            grouped_file_info_df = file_info_df.groupby("Chip", dropna=False)

            expected_memory = (
                excel_df[excel_df["File Name"] == file_name]["Expected Memory"]
                .values[0]
                .split(",")
            )
            ...
            msg_info,is_valid = self.memory_validation_check_expected_and_finding(expected_memory, list(grouped_file_info_df.groups.keys()))            
            # for chip in grouped_file_info_df.groups.keys():
            ...
            #         print_message(
            #             "ok",
            #             f"In {file_name} file, {chip} is in the expected memory list",
            #         )
                
            #         #indexes = list(indexes)
            #         #file_info_df["Verification"].iloc[indexes]="OK"
                    
            #     else:
            #         print_message(
            #             "warning",
            #             f"In {file_name} file, {chip} is not in the expected memory list. Expected memory list: {expected_memory}",
            #         )
            #         #indexes = list(indexes)
            #         #file_info_df["Verification"].iloc[indexes]="NOK"
            #         is_valid = False
                    
            excel_df["Verification"].iloc[index] = str(is_valid) 
            excel_df["Status"].iloc[index] = msg_info     
    ...
    def get_file_info(self, file_name: str):
        ...
        link_result_df_for_sec["in_out_is_equal"] = (
            link_result_df_for_sec["[in] Section"]
            == link_result_df_for_sec["[out] Section"]
        )
        not_equals_table = link_result_df_for_sec[
            ~link_result_df_for_sec["in_out_is_equal"]
        ]

        self.map_parser.init_tables(
            [
                "locate_result_sections",
                "locate_result_combined_sections",
                "locate_result_symbols_address",
            ]
        )
        locate_result_sections_df = self.map_parser.tables["locate_result_sections"][
            ["Chip", "Section", "Size (MAU)", "Space addr"]
        ]
        locate_result_sections_df_renamed = locate_result_sections_df.rename(
            columns={"Section": "[out] Section"}
        )
        locate_result_combined_sections_df = self.map_parser.tables[
            "locate_result_combined_sections"
        ]
        symbols_df = self.map_parser.tables["locate_result_symbols_address"][
            ["Name", "Space addr"]
        ]
        ...
        if not not_equals_table.empty:
            ...
            print_message(
                "warning",
                f"In {file_name} file, there is conflicts in this sections:\n"
                + not_equals_table.to_string(),
                True,
            )
            not_equal_link_res_sec = pd.merge(
                not_equals_table,
                locate_result_sections_df_renamed,
                on="[out] Section",
                how="left",
            )
            search_on_combined_for_not_equal = not_equal_link_res_sec[
                not_equal_link_res_sec["Chip"].isna()
            ]
            if not search_on_combined_for_not_equal.empty:
                search_on_combined_for_not_equal = (
                    search_on_combined_for_not_equal.drop(columns=["[in] Section"])
                )
                search_on_combined_for_not_equal = (
                    search_on_combined_for_not_equal.rename(
                        columns={"[out] Section": "[in] Section"}
                    )
                )
                search_on_combined_for_not_equal = search_on_combined_for_not_equal[
                    ["[in] Section", "in_out_is_equal"]
                ]
                not_equal_link_res_and_combined_sec = pd.merge(
                    search_on_combined_for_not_equal,
                    locate_result_combined_sections_df,
                    on="[in] Section",
                    how="left",
                )
                not_equal_link_res_and_combined_sec[
                    "Chip"
                ] = not_equal_link_res_and_combined_sec["[out] Section"].apply(
                    lambda x: (
                        locate_result_sections_df.loc[
                            locate_result_sections_df["Section"] == x, "Chip"
                        ].values[0]
                        if x in locate_result_sections_df["Section"].values
                        else np.NaN
                    )
                )
                not_equal_link_res_and_combined_sec[
                    ...
                    lambda x: (
                        locate_result_sections_df.loc[
                            locate_result_sections_df["Section"] == x, "Space addr"
                        ].values[0]
                        if x in locate_result_sections_df["Section"].values
                        else np.NaN
                    )
                )
                not_equal_link_res_and_combined_sec["Space addr"] = (
                    not_equal_link_res_and_combined_sec.apply(
                        lambda row: (
                            "0x"
                            + hex(
                                int(row["Group addr"], 16)
                                + int(row["[out] Offset"], 16)
                            )[2:].zfill(8)
                            if not pd.isna(row["Group addr"])
                            else np.NaN
                        ),
                        axis=1,
                    )
                )
                not_equal_merged = not_equal_link_res_and_combined_sec[
                    [
                        "Chip",
                        "Space addr",
                        "[in] Size (MAU)",
                ...
                not_equal_merged = not_equal_link_res_and_combined_sec.rename(
                    columns={"[in] Size (MAU)": "Size (MAU)"}
                )
            else:
                not_equal_merged = not_equal_link_res_sec
            print_message(
                "warning",
                "Could be 2 or more symbol in the same space adrress:\n"
                + str(not_equal_merged["Space addr"].unique().tolist()),
            )

        ############ Locate Result Sections Part  ############################
        merged_link_res_and_sec = pd.merge(
            link_result_df_for_sec,
            locate_result_sections_df_renamed,
            on="[out] Section",
            how="left",
        )
        ...
        search_on_combined = merged_link_res_and_sec[
            merged_link_res_and_sec["Chip"].isna()
        ]
        merged_link_res_and_sec = merged_link_res_and_sec.drop(search_on_combined.index)

        merged_link_res_and_sec = pd.merge(
            merged_link_res_and_sec, symbols_df, on="Space addr", how="left"
        )
        merged_link_res_and_sec["Name"] = merged_link_res_and_sec["Name"].fillna(
            "Not Found in Sections Table"
        )
        ...
        if not search_on_combined.empty:
            ...
            merged_link_res_and_combined_sec = pd.merge(
                search_on_combined,
                locate_result_combined_sections_df,
                on="[in] Section",
                how="left",
            )
            merged_link_res_and_combined_sec["Chip"] = merged_link_res_and_combined_sec[
                "[out] Section"
            ].apply(
                lambda x: (
                    locate_result_sections_df.loc[
            ...
            merged_link_res_and_combined_sec[
                "Group addr"
            ] = merged_link_res_and_combined_sec["[out] Section"].apply(
                lambda x: (
                    locate_result_sections_df.loc[
                        locate_result_sections_df["Section"] == x, "Space addr"
                    ].values[0]
            ...
            merged_link_res_and_combined_sec["Space addr"] = (
                merged_link_res_and_combined_sec.apply(
                    lambda row: (
                        "0x"
                        + hex(
                            int(row["Group addr"], 16) + int(row["[out] Offset"], 16)
                        )[2:].zfill(8)
                        if not pd.isna(row["Group addr"])
                        else np.NaN
                    ),
            ...
            merged_link_res_and_combined_sec["Name"] = merged_link_res_and_combined_sec[
                "Name"
            ].fillna("Not Found in Combined Sections Table")
            merged_link_res_and_combined_sec = merged_link_res_and_combined_sec.rename(
                columns={"[in] Size (MAU)": "Size (MAU)"}
            )
        ...
        if not not_equals_table.empty:
            ...
            if not search_on_combined.empty:
                ...
                all_merged = pd.concat(
                    [merged_link_res_and_sec, not_equal_merged], axis=0
                )
        else:
            if not search_on_combined.empty:
                all_merged = pd.concat(
                    [merged_link_res_and_sec, merged_link_res_and_combined_sec], axis=0
                )
            else:
        ...
        add_func_or_variable_column = self.classify_sections_as_variable_or_function(all_merged)
        all_merged = add_func_or_variable_column
        MapFileTable().save_df_as(
            all_merged, name="file_info_" + file_name, format="html"
        )
        if all_merged["Chip"].isna().all():
            raise ValueError("Kodda bir hata var")
        # show(all_merged)
    ...
    def get_specific_mem_usage(self):
        ...
        try:
            ...
            if user_input_index < 1 or user_input_index > len(memory):
                raise ValueError(
                    "Please enter a number between 1 and " + str(len(memory))
                )
            ...
            filtered_df = base_table[
                base_table["Memory"].str.contains(
                    user_input_index, na=False, regex=False
                )
            ]
            if filtered_df.empty:
                print(
                    "The memory type could not found in the table. Please enter a valid index or memory type."
                )
                return
            print(filtered_df)
            return filtered_df
    ...
    def get_symbol_info(self):  # belirli bir sembolü bulur
        ...
        filtered_locate_res_sections_df = locate_res_sections_table.query(
            "`Space addr` == {}".format(adress.values)
        )
        ...
        cross_references_df = cross_references_table.query(
        ...
        MapFileTable().save_df_as(
            info_df, name="symbol_info_" + symbol_name, format="html"
        )
    ...
    def get_table_from_map_file(self):
        ...
        str_tables = "\n".join(
            [
                str(i + 1) + ". " + table
                for i, table in enumerate(self.map_parser.tables.keys())
            ]
        )

        table_input = input(
            "Enter the table name given below\n" + str_tables + "\n Table number: "
        )
        ...
        try:
            ...
            if table_input < 1 or table_input > len(self.map_parser.tables.keys()):
                raise ValueError(
                    ...
                    + str(len(self.map_parser.tables.keys()))
                )
        except ValueError:
            print(
                "Please enter a number between 1 and "
    ...
    def show_table_gui_and_save(self, table_df, table_name):
        gui_input = input(
            "Do you want to see the table in GUI? After you close the GUI, you will be asked to save the table with changes. (yes/no): "
        )
        if gui_input.lower() == "yes":
            ...
            save_input = (
                input("Do you want to save the table with changes? (yes/no): ")
                if not is_equal
                else input(
                    "There is no change in the table. Do you want to save the table? (yes/no): "
                )
            )

            if save_input == "yes":
                format_input = input(
                    "Enter the type of file you want to save (csv, xlsx, json, html): "
                )
                if format_input in ["csv", "xlsx", "json", "html"]:
                    MapFileTable().save_df_as(
                        gui_df["table_df"],
                        name="changed_" + table_name,
                        format=format_input,
                    )
            ...
            if save_input2.lower() == "yes":
                ...
                if format_input in ["csv", "xlsx", "json", "html"]:
                    MapFileTable().save_df_as(
                        table_df, name=table_name, format=format_input
                    )
                    return table_df
                else:
                    print("Invalid type")
